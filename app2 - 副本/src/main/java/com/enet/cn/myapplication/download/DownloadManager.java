package com.enet.cn.myapplication.download;
...
public class DownloadManager {
	...
	/*package*/ DownloadManager(Context appContext) {
		...
		mContext = appContext;
//        db = DbUtils.create(mContext);	// 这个方法创建的数据库名字默认为：xUtils.db
		db = DbUtils.create(appContext, "enet.db");	// 创建数据库，并指定数据库的名称
		try {
	...
	/**
	 * 通过指定的appId获取下载信息的JavaBean
	 * @param appId
	 * @return
	 */
	public DownloadInfo getDownloadInfoByAppId(String appId) {

		// 找到downloadInfoList中指定appId的下载信息
